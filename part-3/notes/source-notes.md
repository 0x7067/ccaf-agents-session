...
- Form: 40-minute browser-native deck with a simulated demo (hardcoded
  event replay, no server or CLI needed). Two slides have inline knowledge
  checks. The demo replays five key events that prove configuration loading
...
- Practice questions 10, 16, and 17 (the `-p` flag; `--output-format json`
  with `--json-schema`; the independent review instance) supply slides 13
  and 14.
...
- Slides 9–10 carry guide 5.4–5.5 and Domain 3.2: project commands shared
  ...
  skills as folders with `SKILL.md` frontmatter, `context: fork`,
  `allowed-tools`, `argument-hint`, skill-vs-CLAUDE.md (on demand vs always),
  personal variants under different names, and at-rest loading (only the
  description loads until invoked). The plugin caution is Domain 3.2's note.
- Slide 11 carries guide 5.6 and Domain 3.4: planning mode is read-only
...
- Slide 12 carries guide 5.7–5.8 and 5.10 plus Domain 1.7: `/compact`
  (risk of losing numeric values and dates), `/memory`, `--resume`,
  `fork_session`, and fresh-session-with-summary beating stale resumes.
- Slide 13 carries guide 5.9's `-p` non-interactive semantics, structured
  output with `--output-format json` and `--json-schema`. Practice
  question 16's rationale supplies the parse-and-post-inline-comments story.
- Slide 14 carries guide 5.9's session context isolation for review and
  preventing duplicate comments by feeding prior findings back. Practice
  question 17 (the independent review instance) supplies this slide.

### Claude Code documentation

https://code.claude.com/docs/en/

The guide's official-documentation table
...
- **Context degradation signals, scratchpad files, and case-facts blocks.**
  Domain 5 material. Slide 12 only names the `/compact` summarization risk
...
| Hard rule → `PreToolUse` hook | the sprinkler system | Callback to Part 1's hooks: the handbook asks, the sprinkler trips regardless. |
| `@path` imports | the handbook photocopying its own references in | New. Organization by reference; expanded inline. |
...
execution. The demo is fully simulated: no server, no CLI, no credentials.
The presenter opens the HTML file and clicks **Run**.

The simulation replays five key events that tell the night-shift story:

1. **Inventory** — classifies all fixture files into always, conditional,
   on-demand, and skipped. Populates the right-hand panel.
2. **Spawn** — shows `claude -p` being called with `--output-format json`
   and `--json-schema` inside the fixture repo.
3. **Envelope** — the structured result with `maxPrepBatch: 24`, proving
   the handbook and its `@path` import loaded.
4. **Finding** — a high-severity finding (36 portions exceed the 24-portion
   maximum), proving the schema-validated output a CI pipeline would post.
5. **Done** — the run exits cleanly.

The events carry the same payload shapes as the real loader and `claude -p`
output. Each row is clickable to inspect the full JSON.
...
(36 portions against a maximum of 24). The fixture repo exists for reference
and for the terminal-based `npm run nightshift` command; the slide demo does
